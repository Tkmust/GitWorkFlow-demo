...
$(".addVersion").on("click",function(){
	...
	window.location.href=obj.attr("path")+"/appversionadd?id="+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href=obj.attr("path")+"/appversionmodify?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href=obj.attr("path")+"/appinfomodify?id="+ obj.attr("appinfoid");
	}else{
...
$(".viewApp").on("click",function(){
	...
	window.location.href=obj.attr("path")+"/appview?id="+ obj.attr("appinfoid");
});
